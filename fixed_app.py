...
# Merge obs_status for reliability flags
q1_pivot = q1_pivot.merge(
    q1_filtered[['ref_area','time','sex','obs_status']].drop_duplicates(),
    on=['ref_area','time','sex'],
    how='left'
)
...
with tab2:
    ...
    for country in filtered_data['ref_area'].unique():
        ...
        for gender in genders:
            ...
            ax.plot(gender_data['time'], gender_data['informality_rate'],
                    marker='o', label=f"{gender}")
            unreliable = gender_data[gender_data['obs_status'] == 'Unreliable']
            if not unreliable.empty:
                ax.scatter(unreliable['time'], unreliable['informality_rate'],
                           color='red', marker='x', s=100, label=f"{gender} Unreliable")
